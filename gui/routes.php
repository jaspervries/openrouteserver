...
if (mysqli_num_rows($res_routes)) {
    ...
    <table>
    <thead>
    <tr><th>ID</th><th>Naam</th><th>Instantane reistijd [min]</th><th>Afgevlakte reistijd [min]</th></tr>
    </thead>
    <tbody>
    <?php
	while ($row_routes = mysqli_fetch_row($res_routes)) {
		...
		if ($row_routes[1] > (time() - 60*5)) {
			echo '<td style="color:'.$cfg_class_colour[$row_routes[3]].';background-color:#333;">';
		}
		else {
			echo '<td style="color:'.$cfg_class_colour[$row_routes[3]].';background-color:#999;">';
		}
		...
		echo floor($row_routes[5]/60).':'.str_pad($row_routes[5]%60, 2, '0', STR_PAD_LEFT);
		echo '</td>';
		echo '<tr>';
	}
	?>
